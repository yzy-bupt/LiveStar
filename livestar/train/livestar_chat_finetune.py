...
from livestar.model.livestar_chat import (InternVisionConfig,
                                          InternVisionModel,
                                          InternVLChatConfig,
                                          InternVLChatModel)
...
class LazySupervisedDataset(Dataset):
    ...
    def multi_modal_multi_image_get_item(self, data_item):
        ...
        assert ret['attention_mask'].shape == extended_mask.shape, "Shape mismatch!"
        ret['attention_mask'][extended_mask] = False

        # position_ids = ret['attention_mask'].long().cumsum(-1) - 1
        # for i in range(1, ret['attention_mask'].size(1)):
        #     position_ids[:, i] = torch.where(
        #         ret['attention_mask'][:, i] == 0,
        #         position_ids[:, i - 1] + 1,
        #         position_ids[:, i]
        #     )
        position_ids = ret['attention_mask'].long().cumsum(-1) - 1
        mask_zero = ret['attention_mask'] == 0
