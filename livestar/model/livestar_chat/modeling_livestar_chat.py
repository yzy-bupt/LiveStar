...
def find_last_occurrences(labels):
    labels = labels.view(-1)
    last_occurrences = {}  # 存储每一段非-100值的最后位置
    current_segment = []   # 当前非-100段的值
    start_idx = None
    ...
    for i, value in enumerate(labels):
        if value != -100:
            if start_idx is None:
                start_idx = i
            current_segment.append(value.item())
        else:
            if current_segment:
                segment_tuple = tuple(current_segment)
                last_occurrences[segment_tuple] = (start_idx, i - 1)  # 记录最后出现的位置
                current_segment = []
                start_idx = None
    
    if current_segment:
        segment_tuple = tuple(current_segment)
        last_occurrences[segment_tuple] = (start_idx, len(labels) - 1)
...
class InternVLChatModel(PreTrainedModel):
    ...
    def chat(self, tokenizer, pixel_values, question, generation_config, history=None, return_history=False,
             ...
             verbose=False, check_answer=None, self_check=False):
        ...
        if check_answer is not None:
            ...
            labels[:, -answer_len+4:] = input_ids[:, -answer_len+4:]
